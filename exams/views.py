...
def _is_manager(user):
    """TEACHER or SUPER_ADMIN may manage exams."""
    return user.is_authenticated and (
        user.role == User.Role.TEACHER or user.role == User.Role.SUPER_ADMIN
    )


def _get_accessible_exam(request, exam_id):
    """
    Teachers may only access exams they created.
    SUPER_ADMIN may access any exam.
    """
...
def _manager_required(view_func):
    """Decorator: only TEACHER or SUPER_ADMIN."""
...
def exam_list(request):
    if request.user.role == User.Role.SUPER_ADMIN:
        exams = Exam.objects.all().select_related('subject', 'created_by').order_by('-created_at')
    else:
        exams = Exam.objects.filter(created_by=request.user).select_related('subject').order_by('-created_at')
...
def bulk_upload_questions(request, exam_id):
    ...
    if request.method == 'POST':
        ...
        if result['imported']:
            messages.success(
                request,
                f"Successfully imported {result['imported']} question(s)."
            )
        if result['skipped_duplicates']:
            messages.info(
                request,
                f"Skipped {result['skipped_duplicates']} duplicate question(s)."
            )
        ...
        if len(result['errors']) > 10:
            messages.warning(
                request,
                f"... and {len(result['errors']) - 10} more issue(s)."
            )
