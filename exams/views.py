...
def _is_manager(user):
    """
    TEACHER or SUPER_ADMIN may manage exams and subjects.
    """
...
def _get_accessible_exam(request, exam_id):
    """
    Teachers can only access exams they created.
    SUPER_ADMIN can access any exam.
    """
...
def _manager_required(view_func):
    """
    Only TEACHER and SUPER_ADMIN can access exam management pages.
    """
...
def exam_create(request):
    ...
    # Check whether the teacher/admin has subjects available.
    ...
    if not has_subjects:
        ...
        messages.warning(
            request,
            "Add a subject first before creating an exam."
        )
...
def bulk_upload_questions(
    ...
    if request.method == "POST":
        ...
        result = import_questions_from_xlsx(
            exam,
            excel_file,
        )
        ...
        if result["skipped_duplicates"]:
            ...
            messages.info(
                request,
                f"Skipped "
                f"{result['skipped_duplicates']} "
                f"duplicate question(s)."
            )
        ...
        if (
            ...
            and not result["skipped_duplicates"]
        ):
