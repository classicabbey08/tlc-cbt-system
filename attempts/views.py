...
def available_exams(request):
    ...
    return render(request, 'attempts/available_exams.html', {
        'exam_rows': exam_rows,
    })
...
def take_exam(request, attempt_id):
    ...
    # Server-side enforcement: never trust the client's timer alone
    if attempt.is_expired:
        finalize_attempt(attempt, submitted_choice_ids={})
        messages.warning(
            request,
            'Time was already up for this exam. It has been auto-submitted.',
        )
        return redirect('attempts:result_detail', attempt_id=attempt.id)

    questions = attempt.exam.questions.prefetch_related('choices')
    return render(request, 'attempts/take_exam.html', {
        'attempt': attempt,
        'exam': attempt.exam,
        'questions': questions,
...
def result_detail(request, attempt_id):
    """
    Students only see a confirmation message.
    Score, percentage and correct answers are NEVER shown to the student.
    """
    attempt = get_object_or_404(
        ExamAttempt,
        id=attempt_id,
    ...
    return render(request, 'attempts/result_detail.html', {
        'attempt': attempt,
    })
...
def teacher_results(request):
    """Teachers see full scores for attempts on exams they created."""
    attempts = ExamAttempt.objects.filter(
        exam__created_by=request.user,
        status=ExamAttempt.Status.SUBMITTED,
    ).select_related('student', 'exam').order_by('-submitted_at')
    return render(request, 'attempts/teacher_results.html', {
        'attempts': attempts,
    })
...
def admin_results(request):
    """Super Admin sees full scores for every submitted attempt."""
    attempts = ExamAttempt.objects.filter(
        status=ExamAttempt.Status.SUBMITTED,
    ).select_related(
        'student',
    ...
    return render(request, 'attempts/admin_results.html', {
        'attempts': attempts,
    })